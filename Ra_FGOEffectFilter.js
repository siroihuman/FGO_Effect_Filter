...
(() => {
  ...
  const PART_PATHS = [
    ...
    'tools/v1.3.8/parts/source_filter.txt',
    'tools/v1.3.6/parts/p07b.txt',
    'tools/v1.3.6/parts/nl.txt'
  ];
  const EXPECTED_SOURCE_LENGTH = 82926;
  const EXPECTED_UTF8_LENGTH = 92308;
  ...
  async function boot() {
    try {
      ...
      if (!/Version:\s*1\.3\.8/.test(code) || !/const VERSION = '1\.3\.8';/.test(code)) {
